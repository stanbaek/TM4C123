...
enum colororder CurrentColorOrder = GRB; // seems to be the most common (see above), try this first
//enum colororder CurrentColorOrder = RGB; // the Flexible Silicone Neopixel LED Strip (Adafruit #4310) purchased fall 2020 has this encoding order
...
//  DMA_Transfer((uint8_t *)buffer, 1152, 8); // send 12 LEDs with each ping-pong; send 8 ping-pongs
  DMA_Transfer((uint8_t *)buffer, 1920, 12); // send 20 LEDs with each ping-pong; send 12 ping-pongs
}
#else
/*void WS2812_PrintBuffer(void){
  uint32_t i;
