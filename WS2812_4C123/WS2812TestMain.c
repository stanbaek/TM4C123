...
int main(void){ //int main_ChristmasTree(void){
  uint32_t run, maxrun;
  // **** LED string initialization ****
  WS2812_Init();            // set system clock to 80 MHz, initialize SysTick, initialize PE2-0 and make them output
  // **** GPIO Port F initialization ****
                            // activate clock for Port F
  SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R5;
                            // allow time for clock to stabilize
...
/*  // wait for switch pressed
  ...
  while(1){
    ...
    Pattern_Reset();
    WS2812_ClearBuffer();
    while((SWITCHES == (SW1|SW2) && (run < maxrun))){ // repeat while switches released or for about 4 minutes
      Pattern_RainbowWaves(1, NUMCOLS, 0);
      // decrement 'phase' since it should only be incremented once per three calls
      if(phase == 0){
        phase = NUMCOLS - 1;
      } else{
        phase = phase - 1;
    ...
    maxrun = 480;
    Pattern_Reset();
    while((SWITCHES == (SW1|SW2)) && (run < maxrun)){ // repeat while switches released or for about 2 minutes
      Pattern_WhiteWaves(2, 0);
      // decrement 'phase' since it should only be incremented once per three calls
      if(phase == 0){
        phase = 24;
...
void Pattern_SolidRainbow(uint8_t scale, uint8_t row){
  ...
  WS2812_SetCursor(0, row);
  increment = 32;                  // constant amount of color added or subtracted from one LED to the next
  maximum = 224;                   // largest amount of color any LED will reach (i.e. (maximum, 0, 0) is the "reddest" LED)
  if(phase < 8){
    red = maximum - increment*phase;
    green = increment*phase;
    blue = 0;
...
void Pattern_Bulbs(uint8_t scale, uint8_t row){
  ...
  for(i=0; i<endcol; i=i+8){
    if((phase&0x01) == 0){
      ...
      WS2812_AddColor(BULBS[index][0]>>(scale+2), BULBS[index][1]>>(scale+2), BULBS[index][2]>>(scale+2), row);
      WS2812_AddColor(BULBS[index][0]>>scale, BULBS[index][1]>>scale, BULBS[index][2]>>scale, row);
      WS2812_AddColor(BULBS[index][0]>>scale, BULBS[index][1]>>scale, BULBS[index][2]>>scale, row);
      WS2812_AddColor(BULBS[index][0]>>(scale+2), BULBS[index][1]>>(scale+2), BULBS[index][2]>>(scale+2), row);
